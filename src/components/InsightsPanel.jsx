...
import { MAX_RESPONSE_CHARS, SCROLL_HINT_THRESHOLD } from '../utils/constants';
...
function InsightsPanel() {
  ...
  const safeResponse = (raw) => {
    ...
    if (raw.length > MAX_RESPONSE_CHARS) return raw.slice(0, MAX_RESPONSE_CHARS) + '\n\n[Response truncated]';
  ...
  const renderedResponse = useMemo(
    () => response ? renderMarkdown(response) : null,
    [response]
  );
  ...
  return (
    <div className="bg-white rounded-[12px] border border-gray-100 shadow-sm p-6 flex flex-col h-full">
      ...
      {/* Result / Error Area */}
      <div className="mt-5 flex-1" aria-live="polite" aria-busy={isLoading}>
        {isLoading && (
        ...
        {response && !isLoading && (
          <>
            ...
            {response.length > SCROLL_HINT_THRESHOLD && (
